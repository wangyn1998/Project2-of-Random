...
export default class CardTest extends Component {
    ...
    remCard=()=>{
        ...
        fetch(`http://172.17.100.2:3000/users/addstar`,{
            method: 'POST',
    ...
    render() {
        return (
            <View style={{flex:1,width:w,height:h,justifyContent:'center',alignItems:'center',backgroundColor:'#fff'}}>
               ...
               <TouchableOpacity onPress={()=>{Actions.pop();this.props.change();}} style={{position:'absolute',right:40,top:60}}><Icon name='close' color="black"/></TouchableOpacity>
                {
                    [1].map(()=>{
                        if(this.state.num==0){
                            return (
                                <View style={{position:'absolute',alignItems:'center',top:'30%',width:'100%'}}>
                                    <Text style={{fontSize:33}}>{this.props.que}</Text>
                        ...
                        if(this.state.num==1){
                            return (
                                <View style={{position:'absolute',alignItems:'center',top:'30%',width:'100%'}}>
                                    <Text style={{fontSize:33}}>{this.props.que}</Text>
                                    <Text style={{color:'red',marginTop:40}}>{this.props.ans}</Text>
                        ...
                        if(this.state.num==2){
                            return(
                                <View style={{position:'absolute',alignItems:'center',top:'30%',width:'100%',height:'70%'}}>
                                    ...
                                    <TouchableOpacity onPress={()=>{Actions.pop();this.props.change();}} style={{position:'absolute',right:50,bottom:110}}><Text>继续翻牌>></Text></TouchableOpacity>
                                </View>
                            )
                        }
                        if(this.state.num==3){
                            return(
                                <View style={{position:'absolute',alignItems:'center',top:'30%',width:'100%',height:'70%'}}>
                                    <Text style={{fontSize:33}}>{this.props.que}</Text>
                                    <Text style={{color:'red',marginTop:60,fontSize:31}}>{this.props.ans}</Text>
                                    <TouchableOpacity onPress={()=>{Actions.pop();this.props.change();}} style={{position:'absolute',right:50,bottom:110}}><Text>继续翻牌>></Text></TouchableOpacity>
                                </View>
                            )
                        }
